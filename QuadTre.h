...
class QuadTre : public VisualObject
{
private:
...
public:
    ...
    template<class ID, class T>
    void QuadTre<ID, T>::construct()
    {
        mVertices.push_back(Vertex{m_a.first, m_a.second, 0, 0, 0, 1});
        mVertices.push_back(Vertex{m_b.first, m_b.second, 0, 0, 0, 1});
        mVertices.push_back(Vertex{m_c.first, m_c.second, 0, 0, 0, 1});
        mVertices.push_back(Vertex{m_d.first, m_d.second, 0, 0, 0, 1});
    }


    template<class ID, class T>
    void QuadTre<ID, T>::init(GLint matrixUniform)
    {
